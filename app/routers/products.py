...
async def create_product(
    ...
    try:
        ...
        logger.info("creating_product_with_translation", original_name_es=product_data.name_es, original_name_en=product_data.name_en, final_name_es=name_es, final_name_en=name_en)
        product = await DB.create_product(conn=db, name_es=name_es, name_en=name_en, user_email=current_user["email"])
        logger.info("product_created", product_uuid=str(product["uuid"]), admin_email=current_user["email"])
...
async def delete_product(
    ...
    try:
        result = await DB.delete_product_by_uuid(conn=db, product_uuid=product_uuid, user_email=current_user["email"])
        logger.info("product_deleted_successfully", product_uuid=str(product_uuid), product_name=result["name_en"], admin_email=current_user["email"])
        return {"message": "Product successfully deleted", "uuid": result["uuid"], "name": result["name_en"]}
    except PermissionError as e:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(e))
    except ValueError as e:
